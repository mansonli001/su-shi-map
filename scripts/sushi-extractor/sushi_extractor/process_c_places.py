...
PARENT_MAP = {
    'P003': 'P075',  # 百步洪 → 惠州
    'P054': 'P198',  # 广州 → 扬州（不对，广州独立）
    'P056': 'P075',  # 海州花果山古址 → 惠州（不对）
    'P084': 'P089',  # 江陵荆州古城 → 金陵（不对）
    'P097': 'P008',  # 雷州 → 汴京（不对）
    'P216': 'P058',  # 郓州 → 杭州（不对）
}
...
def infer_residence_from_ancient_name(ancient_name, modern_name):
    """从古地名推断居住地"""
    # 如果地名包含"寺"、"观"、"庙"等，通常是游览地
    scenic_keywords = ['寺', '观', '庙', '亭', '阁', '台', '楼', '山', '湖', '江', '河', '泉', '洞', '峰', '岭']
...
def process_c_places():
    ...
    for cp in c_places:
        ...
        print(f"  ✅ {place_id} {ancient_name}: {residence_info['name']}")
