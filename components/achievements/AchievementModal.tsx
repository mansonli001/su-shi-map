...
export default function AchievementModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* 主卡片 — 文人手稿风格 */}
      <main
        className="relative w-full max-w-[400px] ach-parchment-bg ach-double-border shadow-2xl overflow-hidden flex flex-col items-center font-wenkai"
        style={{ animation: 'ach-modal-in 300ms cubic-bezier(0.34,1.56,0.64,1) forwards' }}
      >
        {/* 顶部栏 */}
        <header className="w-full flex justify-between items-center px-6 py-4 sticky top-0 z-20"
          style={{ background: 'rgba(255,248,245,0.4)', backdropFilter: 'blur(12px)' }}
        >
        ...
        {/* 插画区 */}
        <section className="w-full h-[380px] px-6 pt-2 relative">
        ...
        {/* 成就内容 */}
        <section className="w-full px-8 mt-10 text-center flex-grow">
          <h2 className="text-[36px] font-bold mb-3 leading-tight" style={{ color: '#765538' }}>
            {achievement.name}
          </h2>
          <div className="ach-lattice-divider w-1/2 mx-auto mb-6" />
          {/* 诗词块 */}
          <div className="mb-6 text-xl leading-[1.8]" style={{ color: '#4f453d' }}>
        ...
        <section className="w-full px-8 py-8">
          ...
          >
            <div className="flex flex-col items-center" style={{ borderRight: '1px solid rgba(211,196,185,0.3)' }}>
              ...
              <span className="text-xl font-bold" style={{ color: '#7f5539' }}>{userStats.checkinCount}</span>
            </div>
            <div className="flex flex-col items-center" style={{ borderRight: '1px solid rgba(211,196,185,0.3)' }}>
              <span className="text-xs uppercase tracking-wider mb-1" style={{ color: '#81756b' }}>解锁成就</span>
              <span className="text-xl font-bold" style={{ color: '#7f5539' }}>{userStats.achievementCount}/{userStats.totalAchievements}</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-xs uppercase tracking-wider mb-1" style={{ color: '#81756b' }}>苏途进度</span>
              <span className="text-xl font-bold" style={{ color: '#7f5539' }}>{userStats.progressPercent}%</span>
            </div>
          </div>
        </section>

        {/* 底部信息 + QR码 */}
        <footer className="w-full py-8 px-8 flex items-center justify-between"
          style={{ background: 'rgba(236,224,217,0.4)', borderTop: '1px solid rgba(211,196,185,0.3)' }}
        >
          <div className="flex flex-col gap-1">
            <p className="text-lg font-bold leading-tight" style={{ color: '#765538' }}>{achievement.name}</p>
            <p className="text-[13px]" style={{ color: 'rgba(79,69,61,0.8)' }}>{achievement.description}</p>
            <div className="mt-4 pt-4" style={{ borderTop: '1px solid rgba(211,196,185,0.2)' }}>
              {dateStr && (
