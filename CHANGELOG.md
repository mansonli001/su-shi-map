...
用户反馈 `/poems` 在预览环境与线上视觉不一致，表现为：卡片布局和文字层级看起来像“纯文本流”，且预览环境出现脚本报错。

#### 根因分析

1. **预览环境脚本干扰**：`layout.tsx` 无条件注入 `@vercel/analytics`，在本地/非 Vercel 环境会请求 `/_vercel/insights/script.js`，触发资源失败与运行时报错，影响页面稳定性。
2. **列表卡片关键样式对 scoped CSS 依赖过强**：`/poems` 卡片头部布局、类型徽章与链接对齐仅依赖局部样式，导致在部分预览环境中出现排版回退风险。

#### 修复措施

- `app/layout.tsx`
  - 将 Analytics 注入条件改为：**仅 `process.env.VERCEL === '1'` 时加载**，避免本地预览环境注入 Vercel 脚本。
- `app/poems/page.tsx`
  - 对卡片核心布局加**内联兜底样式**（卡片容器、标题行 flex、类型徽章、年份、诗句、行动链接），确保即使局部样式作用域异常，页面也维持与线上一致的视觉结构。
  - 新增 `getTypeBadgeStyle()` 统一输出类型标签样式，保证“词/诗/文/赋/策”颜色与排版稳定。

#### 验证

- `pnpm build` 通过。
- 本地 `next start` 与 `next dev` 均可正常打开 `/poems`（HTTP 200）。
- 页面 HTML 不再包含 `/_vercel/insights/script.js` 注入（本地预览）。
- 预览与线上核心视觉结构对齐：标题行、类型标签、年份、金句、右侧行动链接均恢复稳定渲染。

---

### 97. 诗词库去重 + 补充数据(36篇)深度解读落库 + 优先排序

**日期**：2026-06-12

#### 背景

依据用户提供的手写补充数据（《补充数据.MD》），对诗词库做三件事：①清理重复项与分类错误；②用补充数据替换对应诗词的「深度读」解读；③实现「指定诗词优先展示」排序。

> 说明：补充数据文件经逐条核对实际为 **37 篇**（文件内"32/40/46"计数标记自相矛盾）。经用户确认：本批先按实际内容处理，《纵笔·其一》(惠州，库中缺失且与儋州《纵笔三首》不同诗) 本次跳过，故落库 **36 篇**；缺的后续补。

#### 一、全库去重（基于正文比对，41 组）

首次同名扫描仅发现 10 组，**基于正文内容**复扫发现 **41 组重复**（多为"不同标题/不同类型、相同正文"，属历史数据录入错误）：
- 例：F002《前赤壁赋》=F004《赤壁赋》；C036《念奴娇·赤壁怀古》=C057《念奴娇·大江东去》；C074《卜算子·黄州定慧院寓居作》=C059《卜算子·缺月挂疏桐》；C039《临江仙·夜归临皋》=C060《临江仙·小舟从此逝》；S125《次韵刘景文》=S127《赠刘景文》等。
- **去重规则**（用户确认）：保留「被引用最多」的 ID，其余删除，并把路线/地点对被删 ID 的引用**重定向**到保留 ID（避免断链）。
- 结果：删除 **45 篇**重复 + 重定向 **64 个**引用文件。

#### 二、分类错误修正

- 跨类型重复（词被误存为诗等）随去重一并清理：S055《中秋月》[诗]→C014《阳关曲·中秋月》[词]、S341《宜兴田园》[诗]→C340[词]、S216《磁州》[诗]→C085[词] 等。
- 单独修正：删除 S292《浣溪沙·游蕲水清泉寺》[误标"诗"]（实为该词序言残片），重定向至正词 C038[词]。
- 修正后类型分布：诗 302 / 词 97 / 文 38 / 策 5 / 赋 4。

#### 三、补充数据落库（36 篇深度读）

- 解析《补充数据.MD》的「那时候/我觉得/这个人/金句」→ 映射为 `reading.scene` / `reading.lines[{quote,explain}]` / `reading.person` / `gold_quote`，并补 `age`、`situation`。
- 「人话」引文与说明的拆分：以诗词正文为基准做**繁简无关匹配**（opencc t2s 转简体后比对），修复了 S250《初到黄州》等繁体正文导致的匹配失败；支持散文节选的省略号引文。
- **标题对齐**（用户逐篇确认）：C039 改名《临江仙·夜归临皋》、C081 改名《南乡子·重九涵辉楼呈徐君猷》。
- **新建 2 首**（库中原缺，按权威原文补全，用户确认"只建能确认原文的"）：C200《望江南·超然台作》、S400《汲江煎茶》。
- 36 篇逐条核对：现场/人话/这个人/金句一一对应，无遗漏。

#### 四、优先排序

- 为 36 篇补充诗词写入 `popularity_rank` = 1..36（按补充文件顺序），其余诗词统一 999。
- `app/poems/page.tsx`：列表页改为「精选导读」置顶 + 其余按路线分组（原排序不变）。在「全部」及各分类（词/诗/文/赋）标签下，指定诗词均按 rank 优先展示，未指定的排其后、顺序不变。

#### 五、验证

- 索引 446 条 = 诗词文件 446 个，0 缺失、0 多余。
- 被删 45+1 篇引用 100% 重定向，无新增悬空引用。
- 源 `data-v4` 与 `public/data-v4` 诗词 + 索引完全一致。
- 36 篇优先诗 rank 1-36、全部含 reading 解读数据。
- `pnpm build` 通过；`/poems`、`/poems/C012`、新建 C200/S400、重命名 C039、删除项 C013（回退占位）等路由全 200；C039 SEO 标题已更新为《临江仙·夜归临皋》。
- 排序模拟：全部/词/文/赋 各分类优先区均按 rank 正确置顶。

#### 六、改动文件

| 文件 | 操作 |
|------|------|
| `data-v4/poems/*` + `public/data-v4/poems/*` | 36 篇写解读 / 新建 2 篇 / 删除 46 篇重复 |
| `data-v4/poems-index.json` + `public/data-v4/poems-index.json` | 重建（446 条 + popularity_rank） |
| `public/data-v4/routes/*`、`places/*`（+源） | 引用重定向 |
| `app/poems/page.tsx` | 优先排序 + 精选导读区 |
| `scripts/apply_supplement.py` | 落库/去重/排序一体化脚本 |

#### 七、已知遗留（与本次无关，待后续）

- W019–W068 等 W 系列「文」被 places 引用但库中**从无对应文件**（git 历史即如此，非本次去重所致），属历史悬空引用，建议后续专项补全或清除引用。
- 《纵笔·其一》(惠州) 本次跳过，待补。
